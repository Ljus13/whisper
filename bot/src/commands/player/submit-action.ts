...
export const data = new SlashCommandBuilder()
  ...
  .setDescription('ส่ง Action Code พร้อมหลักฐาน')
...
export async function handleSubmitActionModal(interaction: import('discord.js').ModalSubmitInteraction) {
  ...
  const { data: codeRow } = await supabase
    .from('action_codes')
    .select('id, name, code, expires_at, max_repeats')
    .eq('code', codeStr)
    .maybeSingle()

  if (!codeRow) {
    await interaction.editReply({ content: '❌ ไม่พบรหัส Action นี้ กรุณาตรวจสอบรหัสอีกครั้ง' })
    return
  }
  ...
  await supabase.from('notifications').insert({
    target_user_id: null, // broadcast to all admin
    actor_id: profile.id,
