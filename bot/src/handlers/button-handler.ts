...
export async function handleButton(interaction: ButtonInteraction) {
  ...
  try {
    ...
    if (customId.startsWith('approve_action_')) {
      // TODO: Phase 2
      await interaction.reply({ content: '⏳ (Phase 2) ระบบ Approve ยังไม่พร้อมใช้งาน', ephemeral: true })
      return
    }

    if (customId.startsWith('reject_action_')) {
      // TODO: Phase 2
      await interaction.reply({ content: '⏳ (Phase 2) ระบบ Reject ยังไม่พร้อมใช้งาน', ephemeral: true })
      return
    }
    ...
    if (!interaction.replied && !interaction.deferred) {
      await interaction.reply({ content: '❌ ไม่รู้จัก button นี้', ephemeral: true })
    }
  } catch (error) {
