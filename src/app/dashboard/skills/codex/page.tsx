...
export default async function SkillsCodexPage() {
  ...
  if (!user) redirect('/')
  if (!isStaff) redirect('/dashboard/skills')

  // Auth is cached from layout — only the data queries below hit the DB
  ...
  const [typesRes, pathwaysRes, sequencesRes, skillsRes] = await Promise.all([
    ...
    supabase.from('skill_pathways').select('id, type_id, name, description, overview, bg_url, logo_url, video_url, sort_order, created_at, updated_at').order('name'),
    ...
    supabase.from('skills').select('id, pathway_id, sequence_id, name, description, category, spirit_cost, icon_url, sort_order, created_at, updated_at').order('name'),
  ])
