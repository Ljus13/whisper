...
export default async function RoleplayGuidePage() {
  ...
  const [typesRes, pathwaysRes, sequencesRes] = await Promise.all([
    ...
    supabase.from('skill_pathways').select('id, name, description, overview, type_id').order('name'),
    supabase.from('skill_sequences').select('id, name, seq_number, pathway_id, roleplay_keywords').order('seq_number', { ascending: false }),
  ])
